...
def build_model_main(args):
    ...
    # Go to -models- to see the details of the model!
    from models.registry import MODULE_BUILD_FUNCS 
    assert args.modelname in MODULE_BUILD_FUNCS._module_dict
    # print(f"MODULE_BUILD_FUNCS: {MODULE_BUILD_FUNCS}")
    # print(f"MODULE_BUILD_FUNCS._module_dict: {MODULE_BUILD_FUNCS._module_dict}")
    # print(f"args.modelname: {args.modelname}")
    build_func = MODULE_BUILD_FUNCS.get(args.modelname)
    # print(f"build_func: {build_func}")
...
def main(args):
    ...
    # Custom dataset evaluation (modify the function to change the dataset)
    dataset_train = build_dataset(image_set='train', args=args)
    dataset_val = build_dataset(image_set='val', args=args)
    ...
    for val_images, val_targets in data_loader_val:
        print(f"Dataloader image shape: {val_images.shape}")
        break
    ...
    if args.eval:
        ...
        # with open(args.output_dir + '/results_2_test.json', 'r') as file:
        #     data = json.load(file)

        # # Append the closing square bracket ("]") to the data
        # data.append("]")

        # # Write the modified data back to the JSON file
        # with open(args.output_dir + '/results_2_test.json', 'w') as file:
        #     json.dump(data, file)  

        #  # Per category evaluation
        # coco_obj = COCO("./data/coco/annotations/instances_val2017.json")
        # # dets = coco_obj.loadRes(args.output_dir + f'/results0.json')
        # dets = coco_obj.loadRes(args.output_dir + f'/results_2_test.json')

        # coco_eval = COCOeval(coco_obj, dets, "bbox")  # change bbox if needed
    ...
    if args.test:
        ...
        print(dets)
        print(args.output_dir + f'/results{args.rank}.json')
        coco_eval = COCOeval(coco_obj, dets, "bbox")  # change bbox if needed
        # coco_eval.params.catIds = cat_ids  # 
        coco_eval.evaluate()
        coco_eval.accumulate()
