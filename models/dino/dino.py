...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        """
        # print(samples.tensors.size(), samples.mask.size())
        # stop()
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, poss = self.backbone(samples)
        # print(features[0].tensors.size(), features[0].mask.size())
        srcs = []
        masks = []
        ...
        # print(srcs[0].size(), masks[0].size())
        # print(poss[0].size(), poss[1].size())
        # print(self.num_feature_levels, len(srcs))
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, not_to_xyxy=False, test=False):
        ...
        # print(f"Output logits: {out_logits}")
        # print(f"Output bbox: {out_bbox}")
        # print(f"Output logits shape: {out_logits.shape}")
        # print(f"Output bbox shape: {out_bbox.shape}")
        ...
        # print(f"Topk indexes: {topk_indexes}")
        # print(f"Topk values shape: {topk_values.shape}")
        # print(f"Topk indexes shape: {topk_indexes.shape}")  
        # quit()
...
def build_dino(args):
    ...
    # Go to DINO to see details of the model
    model = DINO(
        backbone,
        transformer,
    ...
    # According to DINO_4scale_swin.py --> Configuration file
    # nms_iou_threshold = -1 means no nms, 
    # num_select = 300 means select top 300 boxes

    # print(args.num_select, args.nms_iou_threshold)
